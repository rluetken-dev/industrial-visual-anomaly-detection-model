...
def parse_arguments() -> Namespace:
    parser = ArgumentParser(
        description="Create the deterministic MVTec AD bottle split manifest."
    )
...
def main() -> None:
    ...
    source_directory = dataset_root / "bottle" / "train" / "good"

    if not source_directory.is_dir():
        raise NotADirectoryError(
            f"Bottle training directory does not exist: {source_directory}"
        )
    ...
    validation_count = round(len(shuffled_paths) * VALIDATION_RATIO)
    validation_paths = sorted(shuffled_paths[:validation_count])
    fitting_paths = sorted(shuffled_paths[validation_count:])

    if set(fitting_paths) & set(validation_paths):
        raise ValueError("Fitting and validation splits overlap.")

    if len(fitting_paths) + len(validation_paths) != len(image_paths):
        raise ValueError("The generated split does not cover every source image.")
    ...
    manifest = {
        ...
        "fitting_images": create_relative_paths(dataset_root, fitting_paths),
